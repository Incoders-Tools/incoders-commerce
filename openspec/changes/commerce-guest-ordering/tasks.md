...
- [~] 6.7 PARTIAL (documented gap, not silently complete): `src/Commerce.Web/e2e/ordering.spec.ts` now covers the public catalogue read, the real verification-request call, and the client-side pre-confirmation submit block against a live backend. The full request→confirm→submit E2E cannot read the issued code back out of `LogOnlyEmailSender` because no Development-gated HTTP seam exists for it (unlike `/internal/test-seed/user`) — adding one is a small additive backend follow-up, deliberately not done here since Unit 6 is web-only and Units 1-5 are closed/merged. See the spec file's doc comment for detail.
