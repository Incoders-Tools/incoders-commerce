...
Synchronization MUST retry unacknowledged operations until acknowledged or explicitly requiring attention. Delivery retries are transport/process attempts and MUST be distinguishable from business effects; duplicate operation delivery MUST NOT repeat acceptance, stock/cash effects, or downstream side effects.
...
- THEN the receiver acknowledges the existing result and applies no second business effect

#### Scenario: Interrupted synchronization

- GIVEN synchronization stops after remote processing and before local acknowledgement
- WHEN connectivity returns
- THEN the operation is retried, converges to one effect, and becomes acknowledged without manual duplicate entry
