...
currently `0011_payments.sql`) against the same direct connection
...
| `Payments__ManuallyRecordedApprovalEnabled` | *(leave unset for fail-closed)* | commerce-payments: **no payment-provider secret exists in this system**, and that absence is fail-CLOSED — `POST /payments` returns 503 (never a silent approval) unless this is explicitly set to `true`, binding the staff-attestation `ManuallyRecordedApproval` gateway. Set it only when staff-recorded (cash/manual) payments are ready to go live; a real provider integration is a future ADR (proposal.md "Out of Scope") that will replace this variable, not extend it. |
